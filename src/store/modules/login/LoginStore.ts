 ...
 * @LastEditors  : HaoJie
 * @LastEditTime : 2020-03-10 14:33:29
...
export default class LoginStore extends VuexModule {
  ...
  public login() {
    if (this.username && this.password) {
      axios.post(`${requestConfig.login.login}?userAccount=${this.username}&userPassword=${md5(this.password)}&entry=1`)
        .then(res => {
          if (res['code'] == -1) {
            msg.warning(res['msg'])
            this.password = '';
          } else if (res['code'] == 0 && res.data.userType == 2) {
